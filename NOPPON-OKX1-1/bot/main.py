...
Usage:
    python -m bot.main run       [--config config.yaml]
...
def cmd_backtest(args):
    ...
    candles = datamod.fetch_history(client, cfg.exchange.inst_id,
                                    cfg.exchange.bar, args.bars)
    ...
    print("\n" + bt.summary(eng, tz_offset=cfg.strategy.tz_offset_hours,
                             months=args.months))
    print(f"\nClosed trades: {len(eng.closed)} | "
          f"open: {len(eng.trades)} | plans: {len(eng.plans)}")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="bot",
                                description="Session Breakout OKX bot")
